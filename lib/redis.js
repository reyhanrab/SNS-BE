...
export const saveDataToRedis = (duration = 10800) => {
  return async (req, res, next) => {
    ...
    res.json = async (body) => {
      const key = getApiV1Path(req.originalUrl); // Using the URL as the key for caching
      try {
        await redisClient.setEx(key, duration, JSON.stringify(body)); // Cache the response for the specified duration
...
export const clearRedisCache = (cacheKeys) => {
  return async (req, res, next) => {
    try {
      // Check if cacheKeys is an array, and clear each key individually
      if (Array.isArray(cacheKeys)) {
        await Promise.all(cacheKeys.map((key) => redisClient.del(key)));
        console.log(`Cache for keys [${cacheKeys.join(", ")}] cleared successfully.`);
      } else {
        // If it's not an array, log an error
        // Delete the cache for the specified key
        await redisClient.del(cacheKeys);

        console.log(`Cache for ${cacheKeys} cleared successfully.`);
      }
